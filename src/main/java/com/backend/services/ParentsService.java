...
public class ParentsService {
    ...
    public boolean parentExist(Long id) {
        return parentsRepository.existsById(id);
    }
    ...
    public void updateParent(Long id, ParentDto parentDto) throws ResourceNotFoundException {
        if (!parentExist(id))
            throw new ResourceNotFoundException("no parent was found with the provided id!");

        var parent = parentsRepository.findById(id).get();
